...
export const SVGDLA: React.FC = () => {
  ...
  } = useAppSelector((state: RootState) => state.svgDla as SVGDLAUIState);
  // Get colorStops from Redux
  const colorStops = useAppSelector(
    (state: RootState) => (state.distanceGradient as any).colorStops,
  );
  ...
  return (
    <>
      ...
      <meta
        name="description"
        content="Generate and export high-quality, scalable vector graphics (SVG) of your DLA fractal creations. Customize the output with various rendering options."
      />
    ...
    entries.forEach((entry) => {
      ...
      if (onlyVisible) {
        const isParentVisible =
          scaledParentX >= 0 &&
          scaledParentX <= CANVAS_WIDTH &&
          scaledParentY >= 0 &&
          scaledParentY <= CANVAS_HEIGHT;
        const isPointVisible =
          scaledPointX >= 0 &&
          scaledPointX <= CANVAS_WIDTH &&
          scaledPointY >= 0 &&
          scaledPointY <= CANVAS_HEIGHT;
      ...
      svgLines.push(
        `<line x1="${scaledParentX}" y1="${scaledParentY}" x2="${scaledPointX}" y2="${scaledPointY}" stroke="#00d8ff" stroke-width="1" />`,
      );
      ...
      if (showCircles) {
        ...
        svgCircles.push(
          `<circle cx="${scaledParentX}" cy="${scaledParentY}" r="${circleRadius}" fill="none" stroke="${color}" stroke-width="1" />`,
        );
      }
    });
  ...
  function generateSVGWithSquares() {
    ...
    entries.forEach((entry) => {
      ...
      if (onlyVisible) {
        const isSquareVisible =
          x >= 0 &&
          x <= CANVAS_WIDTH &&
          y >= 0 &&
          y <= CANVAS_HEIGHT &&
          x + squareSize >= 0 &&
          x + squareSize <= CANVAS_WIDTH &&
          y + squareSize >= 0 &&
          y + squareSize <= CANVAS_HEIGHT;
      ...
      const transform =
        rotations > 0
          ? ` transform="rotate(${rotations} ${scaledX} ${scaledY})"`
          : "";
      ...
      svgSquares.push(
        `<rect x="${x}" y="${y}" width="${squareSize}" height="${squareSize}" ${fillAttr} ${strokeAttr}${transform} />`,
      );
    });
  ...
  function downloadSVG() {
    ...
    console.log(
      `using -> width: ${width}, height: ${height}, viewBox: ${viewBoxX} ${viewBoxY} ${width} ${height}`,
    );
...
const ToolBar: React.FC = () => {
  ...
  const selectedTool = useAppSelector(
    (state: RootState) => (state.svgDla as SVGDLAUIState).selectedTool,
  );
...
const ToolOptions: React.FC = () => {
  const selectedTool = useAppSelector(
    (state: RootState) => state.svgDla.selectedTool,
  );
