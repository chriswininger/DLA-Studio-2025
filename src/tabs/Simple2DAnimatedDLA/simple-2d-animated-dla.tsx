...
import {
  CANVAS_WIDTH,
  CANVAS_HEIGHT,
} from "./simple-2d-animated-dla-constants";
...
const Simple2DAnimatedDLA: React.FC = () => {
  ...
  const colorStops = useSelector(
    (state: any) => state.distanceGradient.colorStops,
  );
  ...
  const shouldShowSpawnShapePreview =
    selectedTool === "spawn-shapes" && !isRunning;
  ...
  const handleMouseDown = useCallback(mouseDown, [
    selectedTool,
    isRunning,
    brushSize,
    brushParticles,
  ...
  return (
    <>
      ...
      <meta
        name="description"
        content="Run a real-time, interactive 2D Diffusion-Limited Aggregation (DLA) simulation. Watch fractal patterns emerge as particles aggregate."
      />
      <div
        className={`dlasim-simple-2d-animated-dla-tab
    ...
    ${selectedTool === "eraser" ? "eraser-active" : ""}`}
      >
        <div className="dlasim-flex-row">
          ...
          {selectedTool === "brush" && (
            <PaintBrushControls isRunning={isRunning} />
          )}
          {/* Eraser controls - only show when eraser tool is selected */}
          {selectedTool === "eraser" && (
            <EraserControls isRunning={isRunning} />
          )}
        </div>
      </div>
    </>
  ...
  function handleSimulateToCompletion() {
    trackUserInteraction(
      "simulate_to_completion",
      "simulation_completion_started",
    );
    dispatch(setIsSimulating(true));
    setSteps(0);
    workerRef.current = new Worker(
      new URL("./dla-worker.ts", import.meta.url),
      { type: "module" },
    );
  ...
  function handleSpawn(newWalkers: { x: number; y: number }[]) {
    if (dlaStateRef.current) {
      console.log(
        ...
        Object.keys(dlaStateRef.current.cluster).length,
      );
      console.log(
        "Cluster positions:",
        Object.keys(dlaStateRef.current.cluster),
      );
      console.log("Spawning walkers:", newWalkers.length);

      trackUserInteraction(
        "spawn_walkers",
        `spawned_${newWalkers.length}_walkers`,
      );
      dlaStateRef.current.walkers = [
        ...dlaStateRef.current.walkers,
        ...newWalkers,
  ...
  function doDraw() {
    ...
    clusterEntries.forEach((entry) => {
      ...
      const color = getColorForDistance(
        colorStops,
        entry.distance ?? 0,
        minDistance,
        maxDistance,
    ...
    dlaStateRef.current.walkers.forEach(
  ...
  function spawnWalkersInBrushRadius(
    centerX: number,
    centerY: number,
    brushSize: number,
    numWalkers: number,
  ...
  function spawnStickyPointsInBrushRadius(
    centerX: number,
    centerY: number,
    brushSize: number,
    numPoints: number,
  ...
  function generateRandomPointInBrushCircle(
    centerX: number,
    centerY: number,
    radius: number,
  ): { x: number; y: number } {
  ...
  function removeWalkersInEraserRadius(
    centerX: number,
    centerY: number,
    eraserSize: number,
  ) {
    ...
    dlaStateRef.current.walkers = dlaStateRef.current.walkers.filter(
      (walker) => {
  ...
  function removeStuckPointsInEraserRadius(
    centerX: number,
    centerY: number,
    eraserSize: number,
  ) {
...
function useSimple2dAnimatedDLAState() {
  const isRunning = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).isRunning,
  );
  const isSimulating = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).isSimulating,
  );
  ...
  const spawnSquareSize = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).spawnSquareSize,
  );
  ...
  const dlaCluster = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).dlaCluster,
  );
  const dlaWalkers = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).dlaWalkers,
  );
  const dlaSteps = useAppSelector(
    (state: RootState) =>
      (state.simple2dAnimatedDla as Simple2DAnimatedDLAUIState).dlaSteps,
  );
  return {
    isRunning,
    isSimulating,
