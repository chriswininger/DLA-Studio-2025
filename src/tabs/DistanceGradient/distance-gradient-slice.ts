...
const initialState: DistanceGradientState = {
  ...
  activeColorId: "1", // Default to first colorStop
};
  ...
  reducers: {
    ...
    removeColorStop: (state, action: PayloadAction<string>) => {
      if (state.colorStops.length > 2) {
        state.colorStops = state.colorStops.filter(
          (stop) => stop.id !== action.payload,
        );
      }
    },
    setActiveColorId: (state, action: PayloadAction<string | null>) => {
...
export function getColorForDistance(
  ...
  if (value >= sorted[sorted.length - 1].position)
